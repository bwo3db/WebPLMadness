<html>
    ...
    <body>
        ...
        if(isset($_COOKIE[$cookie_name])) {
            ...
            $sqlCHECKUSER ="SELECT username FROM maintable WHERE username='john'";
            $result = mysqli_query($con, $sqlCHECKUSER);

            if (mysqli_num_rows($result) > 0) {
                // output data of each row
                while($row = mysqli_fetch_assoc($result)) {
                    $sqlUPDATEUSER = "UPDATE maintable SET bracket = 'INSERTSTRINGHERE', winner= 'INSERTCHAMPHERE' WHERE username = 'john'";
                }
            }
